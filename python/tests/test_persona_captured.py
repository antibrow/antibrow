...
def test_captured_facts_replace_navigator_screen_audio_connection_and_fonts():
    ...
    # host_platform is pinned: captured fonts only replace outright on a
    # Windows host. On a foreign one the allowlist drops what that host cannot
    # draw, so Verdana survives on macOS but not on Linux.
