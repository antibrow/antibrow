...
def personas(count=60, kernel_version="150.0.7871.182"):
...
def test_user_agent_matches_the_kernel_major():
    for version in ("150.0.7871.182", "149.0.7827.201"):
...
def test_generation_is_reproducible_with_a_seeded_rng():
    a = generate_persona(150, "150.0.7871.182", rng=random.Random(1234))
    b = generate_persona(150, "150.0.7871.182", rng=random.Random(1234))
    assert a == b
...
def test_persona_is_frozen_after_first_use(tmp_path):
    first = load_or_generate_persona(tmp_path, "150.0.7871.182")
    for _ in range(5):
        assert load_or_generate_persona(tmp_path, "149.0.7827.201") == first
    assert (tmp_path / "persona.json").exists()


def test_existing_profile_keeps_its_own_kernel_version(tmp_path):
    load_or_generate_persona(tmp_path, "149.0.7827.201")
    later = load_or_generate_persona(tmp_path, "150.0.7871.182")
    assert later.kernel_version == "149.0.7827.201"
    assert later.chrome_major == 149


def test_missing_kernel_version_is_backfilled(tmp_path):
    persona = generate_persona(149, "149.0.7827.201")
    data = persona.to_dict()
    del data["kernelVersion"]
    (tmp_path / "persona.json").write_text(json.dumps(data), encoding="utf-8")

    loaded = load_or_generate_persona(tmp_path, "150.0.7871.182")
    assert loaded.kernel_version == "150.0.7871.182"
    on_disk = json.loads((tmp_path / "persona.json").read_text(encoding="utf-8"))
    assert on_disk["kernelVersion"] == "150.0.7871.182"
...
def test_corrupted_persona_is_regenerated_instead_of_raising(tmp_path):
    ...
    assert load_or_generate_persona(tmp_path, "150.0.7871.182") == persona
...
def test_a_profile_written_by_the_node_sdk_loads_unchanged(tmp_path):
    ...
    persona = load_or_generate_persona(tmp_path, "149.0.7827.201")

    assert persona.seed == "d3a92d0174657118"
    assert persona.kernel_version == "150.0.7871.182"
