...
- The pre-launch exit-IP lookup now asks `https://<server>/api/v1/geo` first and
  ...
  `ANTIBROW_GEO_SERVER`, or per call with the new `server_url` argument on
  `lookup_proxy_geo` / `lookup_direct_geo`. Keyed `relay://` proxies keep using
  the plaintext endpoint: their tunnel cannot carry an HTTPS target.

### Added

- `geo_targets()`, `parse_geo_body()` and `GeoTarget` are exported for callers
  that need to resolve or stub the lookup themselves.
...
- An account whose plan has no cloud sync no longer sends the profile lookup on
  ...
  time. `sync=False` on a plan that does have sync is unaffected.
...
- `resolve_profile_dir()` accepts `skip_server_lookup` for callers that already
  know the account owns no cloud profiles.
