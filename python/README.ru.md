...
| `focus_window` | `bool` | `True` | Получает ли новое окно фокус. `False` открывает его позади того, что сейчас на переднем плане, так что запуск вас не прерывает - окно всё равно есть, просто не в фокусе. Это не headless-режим. Порядок окон определяется ядром, поэтому перед тем, как на это полагаться, установите для профиля последнюю версию ядра. |
...
- **Для Android нужно ядро `151` или новее** - первое, которое несёт поддержку мобильных устройств. Для нового Android-профиля автоматически берётся самое свежее подходящее ядро и устанавливается за вас; профиль, закреплённый за более старым ядром, завершится исключением, а не запустит настольное ядро под маской телефона. `antibrow.kernel_supports_android(version)` отвечает на тот же вопрос напрямую, а `antibrow.android_capable_kernels()` перечисляет подходящие.
...
Заводите отдельный том на каждую архитектуру. Ядра разложены по версиям, а не по процессорам (`<cache_dir>/kernels/<version>/`), поэтому общий том для amd64-хоста и arm64-хоста отдаст неверный бинарник тому из них, кто скачает вторым. В остальном ARM-инстанс здесь обычная цель, а не обходной путь: если ваш парк машин уже работает на ARM, держать отдельную x86-машину ради браузера не нужно.
...
Зашифрованный профиль сначала преобразуется на временной копии, поэтому архив
открывается без ключа; для этого нужны собственный ключ профиля и его ядро, так
что передайте `api_key=`/`server=` и `cache_dir=`. Исходный каталог не меняется, а
если преобразование не произошло, экспорт прерывается, а не записывает файл,
который никто не сможет открыть. Привязанный URL прокси попадает в архив целиком,
вместе с паролем - тот, кому вы передадите файл, получит и эти учётные данные.
