...
describe('captured facts override the generated defaults', () => {
  it('replaces navigator, screen, audio, connection and fonts', () => {
    ...
    // hostPlatform is pinned: captured fonts only replace outright on a Windows
    // host. On a foreign one the allowlist drops what that host cannot draw, so
    // Verdana survives on macOS but not on Linux.
    const config = personaToFpConfig(persona, { label: 'x', timezone: 'UTC', hostPlatform: 'win32' })
